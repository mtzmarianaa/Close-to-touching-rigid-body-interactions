classdef discs
    ...
    properties(Access = public)
        ...
        indCloseChunk % if close information, struct array with indices of chunks considered to be in the 
                               ...
                               %  in the close region
        indC % index of the close chunks in chnkrs, K22 in K (for gamma2) is length(indC)xlength(indC)
        flagReorder % flag to know if this discs object has been reordered (i.e. the chunk order in chnkrs
                            % doesn't really correspond to the chunk order
                            % in chnkrs
        indChnDisc % indices of which chunks in chnkrs belong to which disc       
    end
